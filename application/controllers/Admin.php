			 ...
			 if ($ext == 'xlsx' || $ext == 'xls') {
				 foreach ($object->getWorksheetIterator() as $worksheet) {
					 ...
					 for ($row = 2; $row <= $highestRow; $row++) {
						 ...
						 $TOTAL_PENSION  = $worksheet->getCellByColumnAndRow(8, $row)->getValue();
						 $EPFO  = $worksheet->getCellByColumnAndRow(9, $row)->getValue();
						 $COMMUTATION_FACT  = $worksheet->getCellByColumnAndRow(10, $row)->getValue();
						 $TDS  = $worksheet->getCellByColumnAndRow(11, $row)->getValue();
						 $NET_PENSION_PAYABLE  = $worksheet->getCellByColumnAndRow(12, $row)->getValue();
						 $Month  =  $monthslip;
						 $BENIFICIARY_NAME  = $worksheet->getCellByColumnAndRow(14, $row)->getValue();
						 $NARATION  = $worksheet->getCellByColumnAndRow(15, $row)->getValue();
						 $NARATION2  = $worksheet->getCellByColumnAndRow(16, $row)->getValue();
						 $VERIFIED  = $worksheet->getCellByColumnAndRow(17, $row)->getValue();
					 	$data[] = array(
						 'NAR' => $NAR,
						 'EPF' => $EPF,
						 'PPO' =>$PPO,
